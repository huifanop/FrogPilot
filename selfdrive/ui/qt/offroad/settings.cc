...
//////////////////////////////////////////////////////////////////////////////////////////////
  ...
  FrogPilotButtonsControl *frogpilotBackup = new FrogPilotButtonsControl(tr("FrogPilot 備份"), tr("備份、刪除或還原您的 FrogPilot 備份."), "", frogpilotBackupOptions);
  ...
  connect(frogpilotBackup, &FrogPilotButtonsControl::buttonClicked, [=](int id) {
    ...
    if (id == 0) {
      ...
      if (!nameSelection.isEmpty()) {
        std::thread([=]() {
          frogpilotBackup->setValue(tr("備份中..."));
      ...
      if (!selection.isEmpty()) {
        ...
        std::thread([=]() {
          frogpilotBackup->setValue(tr("刪除中..."));
          QDir dirToDelete(backupDir.absoluteFilePath(selection));
          if (dirToDelete.removeRecursively()) {
            frogpilotBackup->setValue(tr("已刪除!"));
          } else {
            frogpilotBackup->setValue(tr("刪除失敗..."));
          }
          std::this_thread::sleep_for(std::chrono::seconds(3));
          frogpilotBackup->setValue("");
        }).detach();
      ...
      if (!selection.isEmpty()) {
        ...
        std::thread([=]() {
          frogpilotBackup->setValue(tr("還原中..."));
  ...
  FrogPilotButtonsControl *toggleBackup = new FrogPilotButtonsControl(tr("參數備份"), tr("備份、刪除或還原您的參數備份."), "", toggleBackupOptions);
    ...
    if (id == 0) {
      ...
      if (!nameSelection.isEmpty()) {
        std::thread([=]() {
          ...
          if (result == 0) {
            std::cout << "Backup successful to " << fullBackupPath << std::endl;
            toggleBackup->setValue(tr("成功!"));
            std::this_thread::sleep_for(std::chrono::seconds(3));
            toggleBackup->setValue("");
          } else {
            std::cerr << "Backup failed with error code: " << result << std::endl;
            toggleBackup->setValue(tr("失敗..."));
            std::this_thread::sleep_for(std::chrono::seconds(3));
            toggleBackup->setValue("");
          }
        }).detach();
      ...
      if (!selection.isEmpty()) {
        if (!ConfirmationDialog::confirm(tr("您確定要刪除此備份嗎?"), tr("刪除"), this)) return;
        std::thread([=]() {
          toggleBackup->setValue(tr("刪除中..."));
          QDir dirToDelete(backupDir.absoluteFilePath(selection));
          if (dirToDelete.removeRecursively()) {
            toggleBackup->setValue(tr("已刪除!"));
          } else {
            toggleBackup->setValue(tr("失敗..."));
          }
          std::this_thread::sleep_for(std::chrono::seconds(3));
  ...
  auto deleteDrivingDataBtn = new ButtonControl(tr("刪除駕駛數據"), tr("刪除"), tr("此按鈕提供了一種快速且安全的方式來永久刪除所有"
    "裝置上的儲存駕駛錄影和資料。非常適合維護隱私或釋放空間.")
  );
  connect(deleteDrivingDataBtn, &ButtonControl::clicked, [=]() {
    if (!ConfirmationDialog::confirm(tr("您確定要永久刪除所有駕駛錄影和資料嗎?"), tr("刪除"), this)) return;
    std::thread([&] {
      deleteDrivingDataBtn->setValue(tr("刪除影像..."));
  ...
  auto resetTogglesBtn = new ButtonControl(tr("將切換重設為預設值"), tr("重置"), tr("將您的切換設定重設為預設設定."));
  connect(resetTogglesBtn, &ButtonControl::clicked, [=]() {
    if (!ConfirmationDialog::confirm(tr("您確定要完全重設所有切換設定嗎?"), tr("重置"), this)) return;
    std::thread([&] {
      resetTogglesBtn->setValue(tr("重置開關..."));
  ...
  auto flashPandaBtn = new ButtonControl(tr("刷新 Panda"), tr("刷新"), tr("使用此按鈕可排除故障並更新 Panda 裝置的韌體."));
  connect(flashPandaBtn, &ButtonControl::clicked, [=]() {
    if (ConfirmationDialog::confirm(tr("你確定要刷新 Panda?"), tr("刷新"), this)) {
      std::thread([=]() {
        QProcess process;
