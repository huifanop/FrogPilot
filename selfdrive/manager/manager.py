...
def manager_init() -> None:
  ...
  default_params: List[Tuple[str, Union[str, bytes]]] = [
    ("CompletedTrainingVersion", "0" if FrogsGoMoo else "0"),
    ("DisengageOnAccelerator", "0"),
    ("GsmMetered", "0" if FrogsGoMoo else "1"),
    ("HasAcceptedTerms", "2" if FrogsGoMoo else "2"),
    ("IsLdwEnabled", "1"),
    ("IsMetric", "1"),
...
##############################################
    ...
    ("AccelerationPath", "1"),
    ("AccelerationProfile", "3" if FrogsGoMoo else "1"),
    ("AdjacentPath", "1" if FrogsGoMoo else "1"),
    ("AdjacentPathMetrics", "1" if FrogsGoMoo else "1"),
    ("AdjustablePersonalities", "1"),
    ("AggressiveAcceleration", "1"),
    ("AggressiveFollow", "10" if FrogsGoMoo else "12"),
    ("AggressiveJerk", "6" if FrogsGoMoo else "5"),
    ("AlwaysOnLateral", "1"),
    ("AlwaysOnLateralMain", "1" if FrogsGoMoo else "1"),
    ("BlindSpotPath", "1"),
    ("CameraView", "1" if FrogsGoMoo else "2"),
    ("CECurves", "1"),
    ("CENavigation", "1"),
    ("CESignal", "1"),
    ("CESlowerLead", "1"),
    ...
    ("CurveSensitivity", "125" if FrogsGoMoo else "125"),
    ("CustomColors", "0"),
    ("CustomIcons", "0"),
    ...
    ("ExperimentalModeViaLKAS", "1" if FrogsGoMoo else "1"),
    ("ExperimentalModeViaScreen", "0" if FrogsGoMoo else "1"),
    ("Fahrenheit", "0"),
    ("FireTheBabysitter", "1" if FrogsGoMoo else "1"),
    ...
    ("LaneLinesWidth", "4"),
    ("LateralTune", "1"),
    ("LeadInfo", "1" if FrogsGoMoo else "1"),
    ("LockDoors", "0"),
    ("LongitudinalTune", "1"),
    ("LongPitch", "0" if FrogsGoMoo else "1"),
    ("LowerVolt", "0" if FrogsGoMoo else "1"),
    ("MTSCAggressiveness", "100" if FrogsGoMoo else "150"),
    ("Model", "0"),
    ("ModelUI", "1"),
    ("MTSCEnabled", "0" if FrogsGoMoo else "1"),
    ("MuteDM", "1" if FrogsGoMoo else "1"),
    ("MuteDoor", "1" if FrogsGoMoo else "0"),
    ("MuteOverheated", "1" if FrogsGoMoo else "0"),
    ("MuteSeatbelt", "1" if FrogsGoMoo else "1"),
    ("NNFF", "1"),
    ("NoLogging", "0"),
    ("NudgelessLaneChange", "1"),
    ("NumericalTemp", "1" if FrogsGoMoo else "1"),
    ("Offset1", "0"),
    ("Offset2", "7" if FrogsGoMoo else "0"),
    ("Offset3", "10" if FrogsGoMoo else "0"),
    ("Offset4", "20" if FrogsGoMoo else "0"),
    ("OneLaneChange", "1"),
    ("PathEdgeWidth", "20"),
    ("PathWidth", "20"),
    ...
    ("Sidebar", "1" if FrogsGoMoo else "1"),
    ("SilentMode", "0"),
    ("SLCFallback", "0"),
    ("SLCOverride", "1"),
    ("SLCPriority1", "1"),
    ...
    ("StandardFollow", "15"),
    ("StandardJerk", "10"),
    ("StoppingDistance", "3" if FrogsGoMoo else "3"),
    ("TSS2Tune", "1"),
    ("TurnAggressiveness", "150" if FrogsGoMoo else "150"),
    ("TurnDesires", "1" if FrogsGoMoo else "1"),
    ("UnlimitedLength", "1"),
    ("UseSI", "1" if FrogsGoMoo else "0"),
    ("UseVienna", "0"),
    ("VisionTurnControl", "1"),
    ("WheelIcon", "1" if FrogsGoMoo else "0")
  ]
  if not PC:
