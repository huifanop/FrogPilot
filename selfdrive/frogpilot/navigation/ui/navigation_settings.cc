...
void FrogPilotNavigationPanel::cancelDownload(QWidget *parent) {
  if (FrogPilotConfirmationDialog::yesorno("您確定要取消下載嗎?", parent)) {
    std::thread([&] {
      std::system("pkill mapd");
    }).detach();
    if (FrogPilotConfirmationDialog::toggle("需要重新啟動才能重新啟用地圖下載", "馬上重啟", parent)) {
...
void FrogPilotNavigationPanel::removeMaps(QWidget *parent) {
  if (FrogPilotConfirmationDialog::yesorno("您確定要刪除所有下載的地圖嗎?", parent)) {
    std::thread([&] {
