版本HFOP（2023-12-10） 
 ====================== 
 * 將相關設定開放設定：自動啟動ACC的速度設、偵測前車遠離的速度差設定
 * 導航於手機上顯示導航規劃資訊 
 * 於畫面上顯示路口導航轉彎訊息並提供中文語音提醒
 

版本HFOP（2023-11-19） 
 ====================== 
 * 增加跟車距離觸控調整(無其他提醒狀態時，點選畫面左下白色狀態框可調整)
 * 增加車型設定.並依車種修正方向盤按鍵問題(若有問題建議重新啟動試試) 
 * 增加自動啟動ACC設定.並依圖資速限或40為預設 
 * 增加離線地圖
 * 增加錯誤紀錄查詢
 * 增加中文語音提醒設定
    * 可個別啟動綠燈提醒.前車遠離提醒.前車急煞體醒.變換車道盲點提醒 
    
 * 最高時速自動設定 
    * 增加選擇道路類別後，依特定條件自動設定高時速
    * 彎道自動降低最高時速
    * 方向燈啟動時或偵測前車緊急降速時自動微降低最高時速
    * 若有使用導航，會依導航速限資訊自動+10%做為最高時速
  * 使用導航時，若超過圖資速限會自動+10%做為最高時速.(40為下限.120為上限.)
